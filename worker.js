...
function init() {
	...
	addSongs().then(resp => console.log("all songs added")).catch(err => console.error(err));
}

/**
 * Add songs to DB from the file "playlist.json"
 */
async function addSongs() {
	let data = JSON.parse(await fs.readFile("playlist.json", { encoding: "utf8" }));
	let interval = setInterval(() => {
		if (conf.collections && conf.collections.songs) {
			clearInterval(interval);
			insertSongs(data).catch(err => "");
		}
	}, 500);
}
 ...
 * @param {object} data 
 */
async function insertSongs(data) {
	let songs = {};
	for (let i in data) {
		for (let j in data[i]) {
			songs[j] = songs[j] || {};
	...
	await conf.collections.songs.insertMany(Object.values(songs));
}